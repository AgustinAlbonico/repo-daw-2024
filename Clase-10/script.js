document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", (e) => {
    e.preventDefault(); // Previene que el formulario se envie vacio
  ...
  function validateForm() {
    ...
    if (!isValid) {
      showModal(
        "modalError",
        "Errores en el formulario:<br>" + errorMessages.join("<br>")
      );
    } else {
      sendFormData(); // Envía los datos del formulario al servidor
    }
  }
  ...
  function sendFormData() {
    ...
    fetch(url)
      ...
      .then((data) => {
        showModal(
          "modalSuccess",
          "<strong>Suscripción exitosa:</strong><br>" + getFormData()
        );
        saveFormData(data);
      })
      .catch((error) => {
        showModal("modalError", "Error en la suscripción:<br>" + error.message);
      });
  }
    ...
    switch (field.id) {
      ...
      case "contraseña":
        if (!/^(?=.*[a-zA-Z])(?=.*\d).{8,}$/.test(field.value)) {
          errorMessage =
            "Debe tener al menos 8 caracteres, formados por letras y números.";
        }
        break;
      case "repContraseña":
      ...
      case "telefono":
        if (field.value && !/^\d{7,}$/.test(field.value)) {
          errorMessage =
            "Debe ser un número de al menos 7 dígitos, sin espacios, guiones ni paréntesis.";
        }
        break;
      case "direccion":
